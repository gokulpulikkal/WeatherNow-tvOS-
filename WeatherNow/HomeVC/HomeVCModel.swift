...
func getCurrentWeather(lon:Double, lat: Double, onCompletion: @escaping (BaseWeatherModel)->()) {
    ...
    let task = ServiceManager.shared.callService(urlString: url, method: .GET) { (response: BaseWeatherModel) in
...
func getForecastData(lon:Double, lat: Double, onCompletion: @escaping (ForeCastResponse)->()) {
    ...
    let task = ServiceManager.shared.callService(urlString: url, method: .GET) { (response: ForeCastResponse) in
        onCompletion(response)
    } fail: { error in
        print(error)
    }
